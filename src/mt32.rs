...
impl MTRng32 {
    ...
    fn reset_array(&mut self, seed: &[u32]) {
        ...
        for _ in 0..size {
            ...
            self.state[i] = (self.state[i] ^ ((prec ^ (prec >> 30)).wrapping_mul(MAGIC_FACTOR1))) +
                            seed[j] + j as u32;
        ...
        for _ in 0..N - 1 {
            let prec = self.state[i - 1];
            self.state[i] = (self.state[i] ^ ((prec ^ (prec >> 30)).wrapping_mul(MAGIC_FACTOR2))) -
                            i as u32;
            i += 1;
...
mod test {
    ...
    const TEST_VECTOR: [u32; 80] =
        [1067595299, 955945823, 477289528, 4107218783, 4228976476, 3344332714, 3355579695,
         ...
         2899497138, 3818095062, 3030756734];
